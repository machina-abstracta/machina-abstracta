...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Machina-Abstracta thinks for you about the meanings.",
        epilog="Example:  machina-abstracta input_text.txt > output_text.txt"
...
def run():
    """
    $ text | machina-abstracta                        # Accepts text from the pipe
    $ echo "...<text>..." | machina-abstracta         #

    $ machina-abstracta multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="machina-abstracta",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"machina-abstracta: {utterance}.")
        syslog.closelog()
